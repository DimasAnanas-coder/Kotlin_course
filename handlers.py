...
def create_user_step_3_handle(user_type, name):
    while True:
        ...
        elif not is_email_format(email):
...
def start_borrow_book_handle():
    ...
    user: User = User.get_user(choose_user)
    if user.is_limit_of_count_books():
        raise BorrowException(limit_count_books_text)
    if user.has_arrears_book():
...
def borrow_book_step_1_handle():
    while True:
        ...
        if isbn_search == '0':
            ...
            print(search_book_by_isbn_exception_text)
        elif User.get_user(User.get_select_user()).has_user_this_book(isbn_search):
            print(borrow_book_already_text)
        else:
            break
    searched_books: List[Library] = []
    for isbn in Library.get_books_list():
